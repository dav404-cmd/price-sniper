...
def extract_search_deals(to_float, html: str, xpath_structure: dict):
    ...
    for deal in deals:
        # Title
        title_tag = deal.select_one("a.dealCardListView__title--underline")
        ...
        # Price
        price_tag = deal.select_one("span.dealCardListView__finalPrice")
        cleaned_price = to_float(price_tag.get_text()) if price_tag else None

        # Original Price
        orig_price_tag = deal.select_one("span.dealCardListView__listPrice")
        ...
        extracted.append({
            ...
            "store": (deal.select_one("div.dealCardListView__store").get_text(strip=True)
                      if deal.select_one("div.dealCardListView__store") else None),
            "category": None,
            "time_stamp": None,
